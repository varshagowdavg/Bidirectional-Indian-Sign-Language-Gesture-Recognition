...
def ajax(request):
	if request.user.is_authenticated:
		...
		with default_storage.open(bin_path, 'wb+') as destination:
			destination.write(file_obj)
			sound = AudioSegment.from_file(bin_path)
			sound.export(wav_path, format="wav")
			print(f'File Stored @ {wav_path}')
		
		os.remove(bin_path)  # Delete the .bin file
